...
namespace GravityVectorToolKit.CSV.Mapping
{
	public class DeviationCellMapping<T> : ClassMap<DeviationCell> where T : IGeometryUserType
	{
		...
		{
			ImportType<Geometry>();
			Id(x => x.GeoHash).Index("Deviation_GeoHash_Idx").GeneratedBy.Assigned();
			Map(x => x.DistToP).Index("Deviation_Filter_Idx");
			//Map(x => x.PPoint).Column("ppoint").CustomType<T>();
			Map(x => x.RelDistStd).Index("Deviation_Filter_RelDistStd_Idx");
			Map(x => x.RelDistMax).Index("Deviation_Filter_RelDistMax_Idx");
			...
			Map(x => x.DataCount).Index("Deviation_Filter_DataCount_Idx");
